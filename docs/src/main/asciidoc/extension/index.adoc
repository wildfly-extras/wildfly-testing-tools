...
public class HelloWorldTest {
    ...
    @Test
    public void testHello(@ServerResource final URI uri, @ServerResource final HttpClient client) throws Exception {
...
* `java.net.http.HttpClient` - An HTTP client which can be used to communicate with the server.
* `jakarta.ws.rs.client.Client` - A REST client which can be used to make REST requests to a server.
* `jakarta.ws.rs.client.WebTarget` - A resource target which can be used to make REST requests to the deployed application.
...
The extension provides three HTTP client options. Choose based on your testing needs:

* **`HttpClient`**
  - Standard Java HTTP client (no extra dependencies)
  - Good for simple HTTP requests and non-REST endpoints
  - Requires manual URI construction
+
[source,java]
----
@ServerResource
...
@Test
void test() {
    HttpRequest request = HttpRequest.newBuilder(baseUri.resolve("/api/users")).GET().build();
    HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
}
----

* **`WebTarget`**
  - Pre-configured with deployment URI
  - Use with `@RequestPath` for clean, declarative endpoint definitions
  - Built-in JSON/XML support via JAX-RS
+
[source,java]
----
@ServerResource
...
* **`Client`**
  - Use when you need to create multiple targets dynamically
  - Useful for custom client configuration (filters, interceptors, etc.)
+
[source,java]
----
@ServerResource
...
@Test
void test() {
    WebTarget target = client.target(baseUri).path("api/users");
...
=== @RequestPath Qualifier

The `@RequestPath` annotation qualifies URI or WebTarget injection points with a path relative to the deployed application's base URI.
The framework automatically handles slash normalization, so both `@RequestPath("/api")` and `@RequestPath("api")` work correctly.
...
public class OrderServiceTest {
    ...
    @GenerateDeployment
    public static void createDeployment(final WebArchive deployment) {
        deployment.addClass(OrderResource.class);
    }

    // URI injection with path
    @ServerResource
    @RequestPath("/api/orders")
    private URI ordersUri;

    // WebTarget injection with path - pre-configured and ready to use
    @ServerResource
    @RequestPath("/api/orders")
    ...
    public void testGetOrders() throws Exception {
        // ordersUri points to: http://localhost:8080/OrderServiceTest/api/orders
        // ordersTarget is a pre-configured WebTarget for the same URI
    ...
    public void testWithParameter(
            @ServerResource @RequestPath("/api/orders/123") URI orderUri) {
        // Parameter injection also works with @RequestPath
        // orderUri: http://localhost:8080/OrderServiceTest/api/orders/123

        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder(orderUri).GET().build();
        HttpResponse<String> response = client.send(request,
                HttpResponse.BodyHandlers.ofString());
        Assertions.assertEquals(200, response.statusCode());
    }
...
NOTE: Without a deployment, `@RequestPath` appends to the server's base URI (e.g., `http://localhost:8080/api`). This is useful for accessing management endpoints or server-level resources.

==== Domain Mode

In domain mode, combine `@RequestPath` with `@DomainServer` to target specific servers:
...
public class DomainOrderTest {
    ...
    public void testDomainDeployment() {
        // ordersTarget points to server-one's deployment
        try (Response response = ordersTarget.request().get()) {
            Assertions.assertEquals(200, response.getStatus());
        }
    }
...
|`wildfly.server.args`
|String
|Arguments to pass as server arguments, e.g. `--stability=preview`
|None
...
`wildfly.server.args`::
Arguments to pass the server when starting the process. Multiple arguments can be space-separated. Use quotes for
arguments containing spaces or special characters.
+
[source,bash]
----
# Single option
mvn test -Dwildfly.server.args="--stability=preview"

# Multiple options
mvn test -Dwildfly.server.args="--stability=preview --server-config=standalone-full.xml"
----
